...
import com.burak.belediyeapp.service.notification.channel.ResolvedPushNotificationHandler;
...
class ResolvedPushNotificationHandlerTest {
    ...
    @InjectMocks
    private ResolvedPushNotificationHandler handler;

    @Test
    void supportsOnlyResolvedStatus() {
        assertThat(handler.supports(ReportStatus.RESOLVED)).isTrue();
        assertThat(handler.supports(ReportStatus.PENDING)).isFalse();
        assertThat(handler.supports(ReportStatus.PROCESSING)).isFalse();
        assertThat(handler.supports(ReportStatus.REJECTED)).isFalse();
    }
    ...
    void deliverSavesNotificationAndSendsPush() {
        ...
        when(municipalityMessageService.buildResolvedPush(any(), any(), any(), any()))
                .thenReturn(new MunicipalityMessageService.PushMessage("Başlık", "Gövde"));

        handler.deliver(reporter, report, "Note");

        verify(notificationRepository).save(any(Notification.class));
        verify(firebasePushClient).send(eq("token-xyz"), eq("Başlık"), eq("Gövde"), eq(Map.of("reportId", "report-1", "type", "REPORT_RESOLVED")));
    }
}
