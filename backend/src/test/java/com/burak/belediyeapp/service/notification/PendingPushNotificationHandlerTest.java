...
import com.burak.belediyeapp.service.notification.channel.PendingPushNotificationHandler;
...
class PendingPushNotificationHandlerTest {
    ...
    @InjectMocks
    private PendingPushNotificationHandler handler;

    @Test
    void supportsOnlyPendingStatus() {
        assertThat(handler.supports(ReportStatus.PENDING)).isTrue();
        assertThat(handler.supports(ReportStatus.RESOLVED)).isFalse();
        assertThat(handler.supports(ReportStatus.PROCESSING)).isFalse();
        assertThat(handler.supports(ReportStatus.FORWARDED)).isFalse();
    }
    ...
    void deliverSavesNotificationAndSendsPush() {
        ...
        when(municipalityMessageService.buildPendingPush(any(), any(), any()))
                .thenReturn(new MunicipalityMessageService.PushMessage("Başlık", "Gövde"));

        handler.deliver(reporter, report, "Note");

        verify(notificationRepository).save(any(Notification.class));
        verify(firebasePushClient).send(eq("token-xyz"), eq("Başlık"), eq("Gövde"), eq(Map.of("reportId", "report-1", "type", "REPORT_PENDING")));
    }
}
